...
$(document).ready(function(){
  ...
  $(".share-menu").on("click","a",function(){
    ...
    if(type == "twitter"){
      var url = "https://twitter.com/intent/tweet?text=Aprendendo%20seletores%20CSS%20em%20portugues?%20Veja%20essa%20ferramenta:%20CSS%20Diner&hashtags=css,cssdiner,webdev&url=http%3A%2F%2Fitxpertz.github.io%2Fcss-diner-ptbr%2F&via=itxpertzbr";
    } else if (type == "facebook") {
      var url = "https://www.facebook.com/sharer.php?src=sp&u=http%3A%2F%2Fitxpertz.github.io%2Fcss-diner-ptbr%2F";
    } else if (type == "email") {
      var url = "mailto:?subject=Veja+essa+ferramenta+CSS+Diner&body=E'+um+jogo+para+aprender+seletores+CSS.%0D%0A%0D%0AVoce+pode+testar+em+at+http://itxpertz.github.io/css-diner-ptbr";
    }
  ...
  $(".mesa-wrapper,.mesa-edge").css("opacity",0);

  buildLevelmenu();

  setTimeout(function(){
    loadLevel();
    $(".mesa-wrapper,.mesa-edge").css("opacity",1);
  },50);
});
...
function resetTable(){
  ...
  var tableWidth = $(".mesa").outerWidth();
  $(".mesa-wrapper, .mesa-edge").width(tableWidth);
}
...
function fireRule(rule) {
  ...
  * On 03/17/14
  *
  * Allow [div][.mesa] to preceed the answer.
  * Makes sense if div.mesa is going to be included in the HTML viewer
  ...
  */
  ...
  // var baseTable = $('.mesa-wrapper > .mesa, .mesa-wrapper > .nametags, .mesa-wrapper > .mesa-surface');
  var baseTable = $('.mesa');
  ...
  try {
    $(".mesa").find(rule).not(baseTable);
  }
  catch(err) {
    rule = null;
  }

  var ruleSelected = $(".mesa").find(rule).not(baseTable);            // What the correct rule finds
  var levelSelected = $(".mesa").find(level.selector).not(baseTable); // What the person finds
...
function winGame(){
  $(".mesa").html('<span class="winner"><strong>You did it!</strong><br>You rock at CSS.</span>');
  addNametags();
  finished = true;
  resetTable();
}
...
function addNametags(){
  $(".nametags *").remove();
  $(".mesa-wrapper").css("transform","rotateX(0)");
  $(".mesa-wrapper").width($(".mesa-wrapper").width());
  ...
  $(".mesa-wrapper").css("transform","rotateX(20deg)");
}
...
function loadLevel(){
  ...
  setTimeout(function(){
    $(".mesa " + level.selector).addClass("strobe");
    $(".pop").removeClass("pop");
  },200);
